# Getting Started with HPC

!!! info

    The following pages have been written for the latest CX3 system version, i.e. CX3 Phase 2. If you are looking for information on the CX3 Legacy system, please see our [CX3 Legacy guide](../legacy-systems/cx3-legacy.md).

## Additional Links

* What is High Performance Computing? (from the [main HPC page](../hpc-index.md))
* What HPC clusters are provided by the RCS and what hardware do they run on? ([HPC Clusters Specification](../cluster-specification.md))
* How to connect to the service ([Using SSH](./using-ssh.md))
* Where you can store your data ([Data Management on HPC](./data-management-on-hpc.md))
* How to access software/applications installed on the HPC service ([Accessing Software](./accessing-software.md))
* Understand how to write a job script and submit it to the queue ([Running your first job](./running-your-first-job.md))
* Please see the [Support and Training](../../support/support-index.md) page for how to get help in using the HPC facility including details on an Introduction to HPC at Imperial College provided by the Graduate School.

Please acknowledge our contributions by citing us in your research. See [here](../../support/citing-rcs.md) for our suggested template.