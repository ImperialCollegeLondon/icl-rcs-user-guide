...
####Kernels
After some testing we have identified that at some point the behaviour of Jupyter kernels changed. As of now, kernels are stored in `.local` inside your `$HOME` directories meaning some older environments will not carry over to the new service. In order to solve this, you will need to re-run the following command **inside** of the Conda environment you wish to use on the new service:<br>
