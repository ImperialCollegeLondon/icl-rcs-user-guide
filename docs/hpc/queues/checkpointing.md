...
!!! warning
    DMTCP does not currently work on multi-node jobs on CX3 Phase 2. We are working to identify a solution.

## Loading DMCTP into your environment
DMCTP is provided by Easybuild so you can load it as a module. Please ensure the selected version matches the current toolchain used for the rest of the job (if any). For example, here we are using the GCCcore 13.2.0 tool chain 
...
## dmtcp_coordinator
A dmtcp_coordinator process must be started before dmtcp is able to handle checkpoints. There should be exactly one dmtcp_coordinator per job. The coordinator will then be able to communicate with all other nodes to run dmtcp_checkpoint and dmtcp_restart which are set via the `DMTCP_HOST` and `DMTCP_PORT` environment variables. While the dmtcp_coordinator can be started manually it is generally simpler to use the integrated launcher unless you want to manually control checkpointing.
...
Putting this all together we can build a simple PBSPro script for running a C binary. You can replace a.out with your application or binrary.
...
DMTCP_HOST=string
Hostname where the cluster-wide coordinator is running. Currently this should be left as localhost while we resolve issues with multi-node jobs. (default: localhost; dmtcp_checkpoint, dmtcp_restart only)
