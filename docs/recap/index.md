...
Through ReCAP, the RCS department now deploys its resources via an allocation system. While the storage and compute resource allocations are provided by us, these allocations are set up and managed independently from us by individual [research groups](./research-groups.md). This is an important distinction to understand if you are the manager of a research group, so please read the relevant documentation on them as well as [allocation management](allocations.md).
